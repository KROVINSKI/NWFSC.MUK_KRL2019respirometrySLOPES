...
## Second Trial,   Round 1 on 2019-10-29 ended 14:10:00
  ((Time >= as.POSIXct("2019-10-29 18:50:00", tz = "UTC")) 
   & (Time < as.POSIXct("2019-10-29 19:05:00", tz = "UTC"))) ~ "T3Round1",  
...
## Second Trial,   Round 7 on 2019-10-29 ended 15:40:00
  ((Time >= as.POSIXct("2019-10-29 15:25:01", tz = "UTC")) 
   & (Time < as.POSIXct("2019-10-29 14:40:00", tz = "UTC"))) ~ "T3Round7", 
...
## Second Trial,   Round 8 on 2019-10-29 ended 15:55:59
  ((Time >= as.POSIXct("2019-10-29 16:14:01", tz = "UTC")) 
   & (Time < as.POSIXct("2019-10-29 16:29:59", tz = "UTC"))) ~ "T3Round8",
...
# 13.) Investigating slope, R squares and outliers

Determinining in-house thresholds for outlierness
X axis is the number of times I "sampled" with the presense


outlier analysis rules..
# justification checks - outlier
# two mechanisms for cause of highter residuals- bubbles on the sensor- if there's a brief spirt of activity with a CO2 bubble
...
# apply the foritfy model to a trial
...
## 13.a Determining True Start Time

```{r }

trial1KRILL <- filter(krillslopes, TrialID.x == "Trial01")

ggplot(trial1KRILL, aes(x = Time, y = oxygen, colour = KrillID)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") +
  #scale_y_continuous(breaks=seq(250,950,by=50))   +
  ggtitle("Trial01 Oxygen mg/L Accross all Treatments") +
  facet_wrap(~ Treatment) + 
   # geom_hline(yintercept = c(.8), colour = "red") +
  theme_bw()
...
## 1x. this is broken plot Trial01 Oxygen mg/L Accross all Treatments (start data at 1500)
```{r Trial01 Oxygen mg/L Accross all Treatments (start data at 1500)}


# Trying to Select by Time

trial1KRILL.time <- trial1KRILL %>% filter(Time>=as.POSIXct("2019-10-28 15:00:01", tz="UTC") & Time<=as.POSIXct("2019-10-28 16:30:01", tz="UTC")) %>%
  filter(Treatment %in% c("CUR", "CHG", "TMP", "n/a"))

ggplot(trial1KRILL.time, aes(x = Time, y = oxygen, colour = KrillID)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") +
  #scale_y_continuous(breaks=seq(250,950,by=50))   +
  ggtitle("Trial01 Oxygen mg/L Accross all Treatments (start data at 1500)") +
  facet_wrap(~ Treatment) +
   # geom_hline(yintercept = c(.8), colour = "red") +
  theme_bw()
...
```{r}
# linear regression plot of the oxygen over time
# the residuals from the regression line
...
# creating my own linear model
Trial1Model <- lm(trial1KRILL$oxygen ~ trial1KRILL$delta_t)

fitTrial1Mod <- lm(oxygen ~ delta_t, data = trial1KRILL)

autoplot(fitTrial1Mod )
...
modTrial1 <- lm(trial1KRILL$oxygen ~ trial1KRILL$delta_t)
modTrial1 <- fortify(modTrial1)
...
#KrillR11noOUT <- mod.df.KrillR11[mod.df.KrillR11$.cooksd <= 4/(length(dfTrial01_KrilLR11$KrillID)), ]

KrillR11noOUT <- dfTrial01_KrilLR11[mod.df.KrillR11$.cooksd <= 4/(length(dfTrial01_KrilLR11$KrillID)), ]
# any set of true false value can be passed to another dataframe so long as it's the same length
# using one dataframe to filter another 
mod.df.KrillR11$.cooksd <= 4/(length(dfTrial01_KrilLR11$KrillID))

#create a plot for Krillr11 without outliers to see it's r squr 
# create a plot for Krillr11 with

# 
# ggplot(modTrial1, aes(x = trial1KRILL$delta_t, y = trial1KRILL$oxygen)) + geom_point()
# 
# ggplot(modTrial1, aes(x = .fitted, y = .resid)) + geom_point(size =1)

```


# 2x. Comparing the scatterplots of KrillR11 with and without outliers
```{r}
...
allpts <- ggplot() + geom_point(data = dfTrial01_KrilLR11, aes(x = dfTrial01_KrilLR11$delta_t, y = dfTrial01_KrilLR11$oxygen, color = "red" ),size=1)
allpts


# allpts <- ggplot(dfTrial01_KrilLR11, aes(x=dfTrial01_KrilLR11$delta_t, y=dfTrial01_KrilLR11$oxygen)) + geom_point()
...
# outliersshown <- allpts + ggplot(KrillR11noOUT, aes(x=KrillR11noOUT$delta_t, y=KrillR11noOUT$oxygen)) + geom_point()
# 
# outliersshown 


par(mfrow = c(1,2))
plot(KrillR11noOUT$delta_t, KrillR11noOUT$oxygen)
plot(dfTrial01_KrilLR11$delta_t, dfTrial01_KrilLR11$oxygen)
...
ggplot(trial1KRILL, aes(x= delta_t, y =oxygen )) + geom_point(size =1)

## definition for outliers in a linear regression- our loose definition
## residuals higher than .03 across trials? 
...
trial1KRILL$lmresult <- (lm(trial1KRILL$oxygen ~ trial1KRILL$delta_t))
...
```{r}
# select a delayed start time
# find a way to move around the oxygen over delta_t relationship
# explore residuals from the slope line 
# the slope is per .00min per mg per liter


# create a vector of the residual from the Sapply slope function
...
# 13.) Plots of Slopes- Investigating the time series for each trial
## 13.a Determining True Start Time

```{r }

krillslopesINVESTG <- merge(dSlopes, krillslopes , by="KrillID")
trial1KRILL <- filter(krillslopesINVESTG, TrialID == "Trial01")


# krillslopes$slope <- krillslopes$slope.x
# krillslopes$rsq <- krillslopes$rsq.x
# krillslopes$oxygenmgL <- krillslopes$oxygen 
# krillslopes$treatment <- krillslopes$Treatment.x
...
dim(krillslopesINVESTG)

trial1KRILL.time <- filter(trial1KRILL, Time < "2019-10-28 15:30:00")

ggplot(filter(trial1KRILL, Time < "2019-10-28 15:30:00"), aes(x = Time, y = oxygen, colour = KrillID)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") +
  #scale_y_continuous(breaks=seq(250,950,by=50))   +
  ggtitle("Trial01 Oxygen mg/L Accross all Treatments 10 Minute Delay Start Time") +
  facet_wrap(~ Treatment.x) + 
   # geom_hline(yintercept = c(.8), colour = "red") +
  theme_bw()
