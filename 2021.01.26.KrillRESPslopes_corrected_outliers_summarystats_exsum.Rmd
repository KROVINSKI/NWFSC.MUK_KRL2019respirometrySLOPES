...
2021.01.13 to Do List:
[x-I think_] Remove Blanks from all treatments 
[x_] transform the units for more "regular"- not extremely small numbers

[_] display the units that slopes are in to factor the oxygen present, 
                                            the wet weight of the krill 
                                            over time
[_] pairwise comparison - reorder the sequence to get all the looks
[_] summary information 80% time per trial - table of time under observation

[_] include in plots the time overall to help visual what that slope means
[_] convert delta_t to hours 
[_] convert wet weight to grams 
...
write.table(UniqueSensorNames, file = "2021.01.20_dRESPmsr_UniqueSensorNames.csv")
...
![Creating Oxygen Value- Understanding Units](/Users/katherinerovinski/GIT/NWFSC.MUK_KRL2019respirometrySLOPES/Picture2_oxygenpervialRESPmsr.png)
Desired units micrograms 

![Oxygen Value- Microgram Units](/Users/katherinerovinski/GIT/NWFSC.MUK_KRL2019respirometrySLOPES/Picture3_oxygenpervialRESPmsr.png)

```{r Units Correction}

dRESPmsr$oxygen <- (dRESPmsr$oxygen*1000)

```


# 6.a) DO correction QA
...
# 7.) Included Treatments
```{r}
dRESPmsrGOODtreatments <- dRESPmsr %>% filter(Treatment %in% c("CUR", "CHG", "TMP"))

dRESPmsr_withblanks <- dRESPmsr

dRESPmsr <- dRESPmsrGOODtreatments

```


# 8.) DO timeseries Plots

*Initial Dataframe dRESPmsr Plots, 80% DO, Oxygen Per Vial, Actual DOmg/L*
...
### 8.1a) PLotting Percent DO filtering Trial 1
```{r 8.1a) PLotting Percent DO filtering Trial 1}
...
```
### 8.1a2) PLotting Percent DO filtering Trial 1 Colored by Round
```{r 8.1a2) PLotting Percent DO filtering Trial 1, warning=FALSE}
...
### 8.1b) Trial 1 Vial Oxygen over Time
```{r 8.1b) Trial 1 Vial Oxygen over Time}

ggplot(T1dRESPmsr, aes(x = Time, y = oxygen, colour = KrillID)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") +
  #scale_y_continuous(breaks=seq(250,950,by=50))   +
  ggtitle("Trial01 Vial Oxygen (µg) over Time") +
  facet_wrap(~ Treatment) + 
  theme_bw()

```

### 8.1c) Trial01 Vial Oxygen over Time Organized by Round (1 of 8)
...
```{r 8.1c) Trial01 Vial Oxygen over Time Organized by Round (1 of 8), warning=FALSE}

ggplot(T1dRESPmsr, aes(x = Time, y = oxygen, colour = RESPRound)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") +
  #scale_y_continuous(breaks=seq(250,950,by=50))   +
  ggtitle("Trial01 Vial Oxygen (µg) per Round (1 of 8)") +
  facet_wrap(~ Treatment) + 
  theme_bw()
...
### 8.2a) PLotting Percent DO filtering Trial 2
```{r 8.2) PLotting Percent DO filtering Trial 2}
...
### 8.2a) Trial 2 Vial Oxygen over Time
```{r 8.2a) Trial 2 Vial Oxygen over Time}

ggplot(T2dRESPmsr, aes(x = Time, y = oxygen, colour = KrillID)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") +
  #scale_y_continuous(breaks=seq(250,950,by=50))   +
  ggtitle("Trial02 Vial Oxygen (µg) over Time") +
  facet_wrap(~ Treatment) + 
  theme_bw()
...
### 8.2c) Trial02 Vial Oxygen over Time Organized by Round (1 of 8)

Second Trial,   Round 1 on 2019-10-28 began 18:50:01
Second Trial,   Round 8 on 2019-10-28 ended 20:50:59

```{r 8.2c) Trial02 Vial Oxygen over Time Organized by Round (1 of 8), warning =FALSE}

ggplot(T2dRESPmsr, aes(x = Time, y = oxygen, colour = RESPRound)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") +
  #scale_y_continuous(breaks=seq(250,950,by=50))   +
  ggtitle("Trial02 Vial Oxygen  (µg) per Round (1 of 8)") +
  facet_wrap(~ Treatment) + 
  theme_bw()
...
### 8.3) PLotting Percent DO filtering Trial 3
```{r 8.3) PLotting Percent DO filtering Trial 3}
...
### 78.3a) Trial 3 Vial Oxygen over Time
```{r 8.3a) Trial 3 Vial Oxygen over Time}

ggplot(T3dRESPmsr, aes(x = Time, y = oxygen, colour = KrillID)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") +
  #scale_y_continuous(breaks=seq(250,950,by=50))   +
  ggtitle("Trial03 Vial Oxygen (µg) over Time") +
  facet_wrap(~ Treatment) + 
  theme_bw()
...
### 8.3c) Trial03 Vial Oxygen over Time Organized by Round (1 of 8)
```{r 8.3c) Trial03 Vial Oxygen over Time Organized by Round (1 of 8), warning = FALSE}

ggplot(T3dRESPmsr, aes(x = Time, y = oxygen, colour = RESPRound)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") +
  #scale_y_continuous(breaks=seq(250,950,by=50))   +
  ggtitle("Trial03 Vial Oxygen  (µg) over Time Organized by Round (1 of 8)") +
  facet_wrap(~ Treatment) + 
  theme_bw()
...
### 8.4) PLotting Percent DO filtering Trial 4
```{r 8.4) PLotting Percent DO filtering Trial 4}
...
### 8.4a) Trial 4 Vial Oxygen over Time
```{r 8.4a) Trial 4 Vial Oxygen over Time}

ggplot(T4dRESPmsr, aes(x = Time, y = oxygen, colour = KrillID)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") +
  #scale_y_continuous(breaks=seq(250,950,by=50))   +
  ggtitle("Trial04 Vial Oxygen  (µg) over Time") +
  facet_wrap(~ Treatment) + 
  theme_bw()
...
### 8.4c) Trial04 Vial Oxygen over Time Organized by Round (1 of 8)
Acclimation period snuck in from the start
Second Trial,   Round 1 on 2019-10-29 began 17:23:01
Second Trial,   Round 8 on 2019-10-29 ended 19:23:59

```{r 8.4c) Trial04 Vial Oxygen over Time Organized by Round (1 of 8), warning=FALSE}

ggplot(T4dRESPmsr, aes(x = Time, y = oxygen, colour = RESPRound)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") +
  #scale_y_continuous(breaks=seq(250,950,by=50))   +
  ggtitle("Trial04 Vial Oxygen  (µg) over Time Organized by Round (1 of 8)") +
  facet_wrap(~ Treatment) + 
  theme_bw()
...
# 9.) Creating Slopes
```{r 9.) Analysis, Respirometry - Creating the Slope Function}
#*********************************
## 9.) Analysis, Respirometry - Creating the Slope Function  
#*********************************  
...
dRESPmsr_withblanks$delta_t <- as.numeric(dRESPmsr_withblanks$delta_t)

cSlopes_withblanks <- by(dRESPmsr_withblanks, dRESPmsr_withblanks$KrillID, function(x){ slope(x$oxygen, x$delta_t)})
...
cSlopes Units
![cSlope (Units)- Micrograms Change per Minute](/Users/katherinerovinski/GIT/NWFSC.MUK_KRL2019respirometrySLOPES/Picture4_cSlopes.png)

ds_blanks has 76 intercepts : includes both blanks and the ambient treatment


### 9.1) dtotal
```{r 9.1) Merging ds and dRESPmsr}
#add column to dslopes thats KrillID
#View(dref)
ds$KrillID <- row.names(ds)
ds_withblanks$KrillID <- row.names(ds_withblanks)
#View(dslopes)
dtotal <- merge(dRESPmsr, ds, by = "KrillID")
dtotal_withblanks <- merge(dRESPmsr_withblanks, ds_withblanks, by = "KrillID")
#View(dtotal)
...
### 9.2) Factor Krill ID
```{r 9.2) Krill ID as a factor & Grouping by Krill ID}
dtotal$KrillID <- factor(dtotal$KrillID)
nlevels(dtotal$KrillID)

dtotal_withblanks$KrillID <- factor(dtotal_withblanks$KrillID)
nlevels(dtotal_withblanks$KrillID)



```
76 is the number of total vials measured
60 is the number of vials when Ambient Treatment is excluded
51 is the number of vials when blanks are removed


### 9.3) Slope & rsq values
Analysis, Respirometry - Creating Slope Functions & Linear Models  
```{r 9.3) Analysis, Respirometry - Creating Slope Functions & Linear Models  }
#*********************************
## 9.3) Creating the ds dataframe
#*********************************
...
dRESPanimal <- dRESPanimal %>% filter(Treatment %in% c("CUR", "CHG", "TMP"))
...
dim(dRESPmsr_withblanks)
dim(dtotal_withblanks)
dim(ds_withblanks)
# dtotal_withblanks <- dtotal_withblanks %>% filter(Treatment %in% c("CUR", "CHG", "TMP", "n/a"))

dSlopes_withblanks <- data.frame(KrillID = levels(dtotal_withblanks$KrillID))

all(is.na(dtotal_withblanks$oxygen))
all(is.na(dtotal_withblanks$delta_t))
# all(is.na(dtotal_withblanks$delta_t^trans))
# all(is.na(dtotal_withblanks$delta_t))

# 
# i <- 76
# mm <- lm(oxygen ~ delta_t,
#                          dtotal_withblanks[dtotal_withblanks$KrillID == dSlopes_withblanks$KrillID[i],],
#                          na.action=na.omit)
#   dSlopes_withblanks$slope[i] <- mm$coefficients[2]
#   dSlopes_withblanks$rsq[i] <- summary(mm)$r.squared
# 
# unique(dSlopes_withblanks$KrillID)
# unique(dtotal_withblanks$KrillID)
# 
# 
# # make a dataframe that will contain all the
# # prior to the linear model
# # create a temporary dmm dataframe
# # assign dtotal with blanks $ krill == dSlopes$Krill to try and identify missing values
# 
...
for(i in 1:length(dSlopes_withblanks$KrillID)){
   mm <- lm(oxygen ~ delta_t,
                         dtotal_withblanks[dtotal_withblanks$KrillID == dSlopes_withblanks$KrillID[i],],
                         na.action=na.omit)
  dSlopes_withblanks$slope[i] <- mm$coefficients[2]
...
krillslopes_withblanks <- merge(dtotal_withblanks, dSlopes_withblanks, by="KrillID")

dSlopes_withblanks$SensorName <- ""

dSlopes_withblanks <- dSlopes_withblanks %>% group_by(KrillID)
dRESPanimal <- read.csv(file = "RespirometryTrials_all.Animal.Info.csv")
dRESPanimal <- dRESPanimal %>% group_by(SensorName)

#|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |

dSlopes_withblanks$KrillID <- as.character(dSlopes_withblanks$KrillID)
dSlopes_withblanks$SensorName <- dRESPanimal$SensorName
dSlopes_withblanks$TrialID <- dRESPanimal$TrialID
dSlopes_withblanks$MOATS <- dRESPanimal$MOATS
dSlopes_withblanks$Treatment<- dRESPanimal$Treatment
dSlopes_withblanks$LoadingLocation<- dRESPanimal$LoadingLocation
dSlopes_withblanks$VialNum<- dRESPanimal$GlassVialNumber
dSlopes_withblanks$TelsonLength<- dRESPanimal$TelsonLength..mm.
dSlopes_withblanks$WetWeight<- dRESPanimal$Size
...
### 9.4) Filtering krillslopes by each Trial
```{r 9.4 Filtering dtotal by each trial}
...
```{r 8.6 bringing all trails back together following 80% DO selection}
...
```{r 8.8 bringing all trails back together following 70% DO selection}
...
### 10.1 Investigating Outliers by Trial- cleaning up Trials Removing AMB from the 80% Threshold
```{r 10.1 Investigating Outliers by Trial}
...
### 10.1a Investigating Outliers by Trial- cleaning up Trials Removing AMB from the 70% Threshold
...
### 10.2a) Investigating Outliers by Trial- cleaning up Trials Removing AMB from the 1hr Threshold
```{r 10.2a Investigating Outliers by Trial}
...
# 13.) Removing Blanks
```{r 13.) Removing Blanks}

# # ''' Trial 01 '''
# T1krillslopes <- droplevels(filter(T1krillslopes, Treatment != "n/a"))
# levels(T1krillslopes$Treatment)
# 
# T1krillslopes1hr <- droplevels(filter(T1krillslopes1hr, Treatment != "n/a"))
# levels(T1krillslopes$Treatment)
# T1krillslopes1hr <- droplevels(filter(T1krillslopes80, Treatment != "n/a"))
# levels(T1krillslopes$Treatment)
# T1krillslopes1hr <- droplevels(filter(T1krillslopes70, Treatment != "n/a"))
# levels(T1krillslopes$Treatment)
# 
# # #|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
# 
# # #|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
# # ''' Trial 02 '''
# T2krillslopes <- droplevels(filter(T2krillslopes, Treatment != "n/a"))
# levels(T2krillslopes$Treatment)
# 
# T2krillslopes1hr <- droplevels(filter(T2krillslopes1hr, Treatment != "n/a"))
# levels(T2krillslopes$Treatment)
# T2krillslopes1hr <- droplevels(filter(T2krillslopes80, Treatment != "n/a"))
# levels(T2krillslopes$Treatment)
# T2krillslopes1hr <- droplevels(filter(T2krillslopes70, Treatment != "n/a"))
# levels(T2krillslopes$Treatment)
# # #|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
# 
# # #|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
# # ''' Trial 03 '''
# T3krillslopes <- droplevels(filter(T3krillslopes, Treatment != "n/a"))
# levels(T3krillslopes$Treatment)
# 
# T3krillslopes1hr <- droplevels(filter(T3krillslopes1hr, Treatment != "n/a"))
# levels(T3krillslopes$Treatment)
# 
# T3krillslopes1hr <- droplevels(filter(T2krillslopes1hr, Treatment != "n/a"))
# levels(T2krillslopes$Treatment)
# T3krillslopes1hr <- droplevels(filter(T2krillslopes80, Treatment != "n/a"))
# levels(T2krillslopes$Treatment)
# T3krillslopes1hr <- droplevels(filter(T2krillslopes70, Treatment != "n/a"))
# levels(T2krillslopes$Treatment)
# 
# 
# # #|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
...
# # #|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
# # ''' Trial 04 '''
# T4krillslopes <- droplevels(filter(T4krillslopes, Treatment != "n/a"))
# levels(T4krillslopes$Treatment)
# 
# T4krillslopes1hr <- droplevels(filter(T4krillslopes1hr, Treatment != "n/a"))
# levels(T4krillslopes$Treatment)
# 
# T4krillslopes1hr <- droplevels(filter(T2krillslopes1hr, Treatment != "n/a"))
# levels(T2krillslopes$Treatment)
# T4krillslopes1hr <- droplevels(filter(T2krillslopes80, Treatment != "n/a"))
# levels(T2krillslopes$Treatment)
# T4krillslopes1hr <- droplevels(filter(T2krillslopes70, Treatment != "n/a"))
# levels(T2krillslopes$Treatment)
# #|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
...
51 Levels is correct for no "Ambient" treatment Krill & No blank vials included
...
Krill Slopes will be displayed in the four datasets
Picture below describes how the corrected slopes are created


cSlopes Units
![Final Slope Values (Units)- Micrograms of Oxygen per Krill Tissue Change per Minute](/Users/katherinerovinski/GIT/NWFSC.MUK_KRL2019respirometrySLOPES/Picture4_FinalSlopeValues.png)

![Examining Slope Values (Units)- Micrograms of Oxygen per Krill Tissue Change per Minute](/Users/katherinerovinski/GIT/NWFSC.MUK_KRL2019respirometrySLOPES/Picture5_FinalSlopeValuesfromcSlope.png)
...
ksSlopes80 <- ksSlopes80 %>% filter(Treatment %in% c("CUR", "CHG", "TMP"))
...
ksSlopes70 <- ksSlopes70 %>% filter(Treatment %in% c("CUR", "CHG", "TMP"))
...
x <- subset(dSlopes_withblanks, dSlopes_withblanks$MOATS == "blank")
...
dSlopes$WetWeight <- as.numeric(dSlopes$WetWeight)*1000
...
Review about Corrected Slopes (units)
[Micrograms of Oxygen per Minute, Per Microgram of Krill Tissue](/Users/katherinerovinski/GIT/NWFSC.MUK_KRL2019respirometrySLOPES/Picture5_correcteddSlopes.png)
...
# 
# dSlopes <- filter(dSlopes, Treatment != "n/a")  
...
write.csv(dSlopes.summary, "2021.01.20_dSlopes_summary.csv")
```
...
# #|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
# Correcting for the size of the animal 
ksSlopes80$WetWeight <- as.numeric((ksSlopes80$WetWeight)*1000)
...
write.csv(ksSlopes80.summary, "2021.01.20_ksSlopes80_summary.csv")
```
...
# #|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
# Correcting for the size of the animal 
ksSlopes70$WetWeight <- as.numeric((ksSlopes70$WetWeight)*1000)
...
write.csv(ksSlopes70.summary, "2021.01.20_ksSlopes70_summary.csv")
```
...
# #|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
ksSlopes1hr$WetWeight <- as.numeric((ksSlopes1hr$WetWeight)*1000)
...
write.csv(ksSlopes1hr.summary, "2021.01.20_ksSlopes1hr_summary.csv")
```
...
write.csv(Slope_Stats, "2021.01.20_Slope_Stats.csv")
...
# intercept is the random factor designated by the one
# random effects is the mean of the MOATS



# what about different rounds on different days?
# starved for same amount of time
# day is nested inside of MOAT- different MOATs sampled for respir rounds in one day
