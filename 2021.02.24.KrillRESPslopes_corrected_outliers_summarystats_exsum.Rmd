...
write.csv(dRESPanimal, file = "2021.02.09_krillanimaldata.csv", row.names = FALSE)
#|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
...
write.csv(dRESPmsr, file = "2021.02.23_respirometrymeasurements.csv", row.names = FALSE)
#|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
...
ztrial01DO <- ggplot(T1dRESPmsr, aes(x = Time, y = percentDO, colour = SensorName)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") +
  #scale_y_continuous(breaks=seq(250,950,by=50))   +
...
ztrial01DO + theme(legend.position="bottom")

ztrial01DO + theme(legend.position="bottom", plot.title = element_text(hjust = 0.5))
...
ggplot(T1dRESPmsr, aes(x = Time, y = percentDO, colour = T1dRESPmsr$RESPRound)) +
  ...
  geom_smooth(method = "lm") +
  #scale_y_continuous(breaks=seq(250,950,by=50))   +
  ggtitle("Trial01 Percent DO placeholder") +
  facet_wrap(~ Treatment) + 
  geom_hline(yintercept = c(.8), colour = "red") +
  theme_bw()
...
ggplot(T1dRESPmsr, aes(x = Time, y = oxygen, colour = KrillID),
  ...
  geom_smooth(method = "lm") +
  #scale_y_continuous(breaks=seq(250,950,by=50))   +
  ggtitle("Trial01 Vial Oxygen (mg) Timeseries") +
  facet_wrap(~ Treatment) + 
  xlab("Time") +
  ylab("Oxygen (mg)") +
  theme_bw()
