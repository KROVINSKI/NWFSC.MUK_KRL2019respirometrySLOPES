...
# 12.) Filtering krillslopes by each Trial
```{r 12. Filtering dtotal by each trial}
...
# 12.x) Filtering krill slopes by each Trial and finding the 70% time
```{r 12.x Filtering dtotal by each trial and finding the 80% DO times, warnings= FALSE}
...
## 12.x. bringing all trials back together following 70% DO selection)
```{r 12.x. bringing all trails back together following 80% DO selection}
...
# 12.x) Filtering krillslopes to give each Trial only 1hr
```{r 12.x Filtering krill slopes by each trial with 1hr cutoff, warnings= FALSE}
...
## 12.x. bringing all trials back together following the 1hr selection)
```{r 12.x. bringing all trials back together following the 1hr selection}
...
## 13.1 Investigating Outliers by Trial- cleaning up Trials
...
###12.1x Investigating Outliers by Trial- timeseries plots showing the four distinct dataframes
```{r 12.1x Investigating Outliers by Trial- timeseries plots}
...
# 13.) Krill Sparkle Function (standard krillslopes dataframe)
...
# 13.) Krill Sparkle Function (80% DO dataframe)
```{r 13.) Krill Sparkle Function}
...
# 13.) Krill Sparkle Function (70% DO dataframe)
```{r 13.) Krill Sparkle Function}
...
# 13.) Krill Sparkle Function (1hr Trial dataframe)
```{r 13.) Krill Sparkle Function}
...
## 12.x. bringing all trials back together 80%, 70% and 1hr
```{r 12.x. bringing all trials back together following the 1hr selection}
...
### 13.1b) Trial 1 Vial Oxygen over Time
```{r 13.1b) Trial 1 Vial Oxygen over Time}
...
# 15.x) Analysis, Respirometry - outliers removed (investigating krillslopes)
```{r 15.) Analysis, Respirometry - Creating the Slope Function}
#*********************************
## 15.) Analysis, Respirometry - Creating the Slope Function  
#*********************************  
...
#having row names be a variable in a column to be able to pull it out for later merging 
ksT1to4$TrialID.x<- row.names(ds)
ks80 $TrialID.x<- row.names(ds)
ks70$TrialID.x<- row.names(ds)
ks1hr$TrialID.x<- row.names(ds)
...
ksT1to4$KrillID<- row.names(ds)
ks80 $KrillID<- row.names(ds)
ks70$KrillID<- row.names(ds)
ks1hr$KrillID<- row.names(ds)
